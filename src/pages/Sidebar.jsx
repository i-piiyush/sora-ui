...
const Sidebar = ({ toggle }) => {
  ...
  return (
    <>
      <div className="h-full md:hidden hidden xl:block  px-10">
          ...
          <div className="pr-4 pb-4">
            {components.map((group, groupIndex) => (
              <div key={group.name} className="mb-8 0 px-5">
                <motion.h1
                  className="text-green-300 font-bold text-base mt-5"
      ...
      {toggle && (
        <div className="bg-zinc-900 h-screen w-full absolute left-0 right-0 ">
            <div className="flex flex-col h-[80vh] w-full overflow-y-auto no-scrollbar">
          <div className="pr-4 pb-4">
            {components.map((group, groupIndex) => (
              <div key={group.name} className="mb-8 0 px-5">
                <motion.h1
                  className="text-green-300 font-bold text-xl mt-5"
                  initial={{ opacity: 0, y: -10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.3 }}
                >
                ...
                <div className="flex flex-col gap-1 items-baseline mt-2 relative ">
                  {group.items.map((item, itemIndex) => (
                    <motion.button
                      type="button"
                      key={item.name}
                      className="flex items-center  gap-1 relative cursor-pointer py-1 bg-transparent border-none outline-none w-full"
                      onClick={() => handleClick(groupIndex, itemIndex)}
                      whileHover={{ scale: 1.03 }}
                      whileTap={{ scale: 0.97 }}
                      style={{ textAlign: "left" }}
